...
TEST(ReadFileIntoMemory, testGoodDecodeResponse)
{
    ...
    auto rc = decode_read_file_memory_resp(
        responseMsg.data(), responseMsg.size(), &retCompletionCode, &retLength);
    ASSERT_EQ(rc, PLDM_SUCCESS);
    ASSERT_EQ(completionCode, retCompletionCode);
    ASSERT_EQ(length, retLength);
}
TEST(ReadFileIntoMemory, testBadDecodeResponse)
{
    uint32_t length = 0;
    uint8_t completionCode = 0;

    // Request payload message is missing
    auto rc = decode_read_file_memory_resp(NULL, 0, &completionCode, &length);
    ASSERT_EQ(rc, PLDM_ERROR_INVALID_DATA);
...
TEST(ReadFileIntoMemory, testGoodEncodeRequest)
{
    ...
    auto rc = encode_read_file_memory_req(0, request, fileHandle, offset,
                                          length, address);
...
TEST(ReadFileIntoMemory, testBadEncodeRequest)
{
    uint32_t fileHandle = 0;
    uint32_t offset = 0;
    uint32_t length = 0;
    ...
    auto rc = encode_read_file_memory_req(0, NULL, fileHandle, offset, length,
                                          address);
